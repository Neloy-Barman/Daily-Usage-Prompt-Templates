``````md
## CONTEXT
I have created a voice agent leveraging the Livekit Agent class. Now I want to create a section of a `Markdown` file that explains how the voice agent code works step by step, so that anyone can understand the agent’s behavior and execution flow. I will provide you with the code file and a draft walkthrough as text. You need to analyze both the code file and the draft walkthrough, and generate a single, polished Markdown section describing the agent’s workflow and the logic and workflow implemented within the agent, basing your explanation primarily on the actual code.
...
## RULES
- Use the draft workflow as reference material.
- Analyze the code structure to identify logical components and execution flow.
...
### Draft Workflow Brief
```plain text
Takes User Input. Pass LLM a specific system prompt and question-answer pair as the user data.
Question is a predefined query and answer implies the user query. LLM returns a structured JSON response with keys sentiment and course, given the response format. From response, we detect the sentiment of the response, If it's negative, then update some required session attributes,
send hang-up response and end the call shutting down the session. If the course is not None, update course attribute in session, prompt for the guidance choice. Finally handover the control to Guidance Agent. If the course is None, then increment all kinds of retry count variables. Check for if it exceeds the Max retry count, if it crossed the limit, then send hang-up response and end the call by shutting down the session. Else send a pre-defined retry prompt.
```
OUTPUT:
````md
## `CourseAgent`
